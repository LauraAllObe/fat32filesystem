...
uint32_t directory_location(int fd_img, bpb_t bpb) {
    ...
    while (token != NULL) {
        ...
        uint32_t offset = 0x100400 + (clusterNum - 2) * clusterSize;
        ssize_t bytesRead = pread(fd_img, buffer, clusterSize, offset);
        ...
        for (uint32_t i = 0; i < bytesRead; i += sizeof(dentry_t)) {
            ...
            // Check if the entry matches the token and is a directory
